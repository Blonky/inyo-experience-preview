const editor=()=>`<div class="phone expanded profile-editor">${status()}${webBar(17)}<div class="editor-scroll"><h2>Your words. Your card.</h2><p>A little more you, a little less guessing.</p><div class="profile-view-switch" role="group" aria-label="Profile view"><button data-profile-view="edit" aria-pressed="true">Details</button><button data-profile-view="preview" aria-pressed="false">Preview card</button></div><form id="profile-form"><div class="editor-fields"><div class="editor-photo"><img src="assets/maya.jpg" alt="Your profile photo"><div><b id="editor-name">Maya, 22</b><small>The face behind the hello.</small><button type="button" class="change-photo" data-change-photo>Change photo</button><input type="file" id="profile-photo" accept="image/jpeg,image/png,image/webp" hidden></div></div><label>Name<input name="name" value="Maya" required maxlength="35" autocomplete="off"></label><label>City<input name="city" value="New York" maxlength="50" autocomplete="off"></label><label>Looking for<select name="intent"><option>A relationship</option><option>Still figuring it out</option><option>Something casual</option></select></label><label>A little about you<textarea name="bio" maxlength="200">Cooking with music on. Long walks that were supposed to be short.</textarea></label><label>What helps you feel close?<textarea name="close" maxlength="160">Real conversation, and room for our own lives.</textarea></label><small class="private-field-note">For Inyo to remember. This part stays off your card.</small></div><div class="editor-preview" hidden><div class="profile shared-profile editor-card">${profileContent('Maya')}</div><p>This is the card someone else sees.<br>Your private preferences stay in your conversation.</p></div><div class="save-line" aria-live="polite">Only you can edit this card.</div><button class="primary" type="submit">Save changes</button></form></div>${home}</div>`;
function setProfileView(view){const form=document.querySelector('#profile-form');if(!form)return;if(view==='preview'){form.elements.name.setCustomValidity(form.elements.name.value.trim()?'':'Add the name you want to share.');if(form.querySelector('[type=submit]').disabled||!form.reportValidity())return;const card=form.querySelector('.editor-card'),values=Object.fromEntries(new FormData(form));card.querySelector('.profile-meta b').textContent=values.name.trim()+', 22';card.querySelector('.profile-meta>span').textContent=values.city.trim();card.querySelector('.profile-meta small').textContent=values.intent==='A relationship'?'Looking for a relationship':values.intent;card.querySelector('.profile-bio').textContent=values.bio;card.querySelector('.album-image').src=showcaseState.photo||state.profilePhoto||'assets/maya.jpg';}form.querySelector('.editor-fields').hidden=view==='preview';form.querySelector('.editor-preview').hidden=view!=='preview';document.querySelectorAll('[data-profile-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.profileView===view)));document.querySelector('.editor-scroll').scrollTop=0;}
document.addEventListener('click',e=>{const b=e.target.closest('[data-profile-view]');if(b)setProfileView(b.dataset.profileView);});
